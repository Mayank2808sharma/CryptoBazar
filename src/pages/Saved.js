...
const Saved = () => {
  ...
  return (
    <section className="w-full md:w-[80%] h-full flex flex-col mt-16 mb-24 relative px-4 md:px-0">
      <div className="w-full min-h-[60vh] py-8 border border-gray-100 rounded overflow-x-auto">
        {savedData ? (
          <table className="w-full table-auto">
            ...
            <tbody>
              {savedData.map((data) => (
                <tr
                  key={data.id}
                  className="text-center text-base border-b border-gray-100 hover:bg-gray-100"
                >
                  <td className="py-4 uppercase flex items-center justify-center md:justify-start px-3">
                    <SaveBtn data={data} />
                    <img
                      src={data.image}
                      alt={data.id}
